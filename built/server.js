...
const aniSync = new Core_1.default({ is_sqlite: true });
...
fastify.get("/seasonal/:type/:season", async (req, res) => {
    ...
    if (type.toLowerCase() === "anime") {
        const data = await aniSync.getSeasonal(AniList_1.Type.ANIME, 8);
        ...
        const data = await aniSync.getSeasonal(AniList_1.Type.MANGA, 8);
        if (season.toLowerCase() === "trending") {
            res.type("application/json").code(200);
            return data.trending;
        }
